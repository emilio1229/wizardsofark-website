...
/** Ensures maps/council exist even if start.sh seed was skipped by a Railway start-command override. */
@Injectable()
export class CatalogueSeedService implements OnModuleInit {
  private readonly logger = new Logger(CatalogueSeedService.name);

  constructor(private readonly prisma: PrismaService) {}

  async onModuleInit(): Promise<void> {
    const [mapCount, councilCount] = await Promise.all([
      this.prisma.arkMap.count(),
      this.prisma.councilMember.count(),
    ]);

    if (mapCount > 0 && councilCount > 0) {
      this.logger.log(`Catalogue already seeded (maps=${mapCount}, council=${councilCount})`);
      return;
    }

    this.logger.warn(
      `Catalogue empty (maps=${mapCount}, council=${councilCount}); running bootstrap seed`,
    );
    await this.seedAll();
    ...
    this.logger.log(`Bootstrap seed complete (maps=${maps}, council=${council})`);
  }
  ...
  private async seedAll(): Promise<void> {
    ...
    for (const map of MAP_SEEDS) {
      ...
      await this.prisma.arkMap.upsert({
        where: { id: map.id },
        create: { ...map, sortOrder },
        update: {
          name: map.name,
    ...
    for (const member of COUNCIL_SEEDS) {
      await this.prisma.councilMember.upsert({
        where: { id: member.id },
        create: {
          ...member,
